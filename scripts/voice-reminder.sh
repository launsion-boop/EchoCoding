...
case "$LEVEL" in
  balanced)
    emit_reminder \
      "<user-prompt-submit-hook>💡 平衡模式：评估本次回复是否需要语音播报。如果有重要信息、阶段性结论或需要用户关注的内容，建议先 echocoding say 一句摘要。纯工具调用或简短确认可以不说。⚠️ 如果需要反问用户或请求确认，必须用 echocoding ask，不能用文字提问。ASK 不是一次性提问：若需求未澄清，请连续多轮 ask；HUD 会话按最后一次互动滑动 60 秒超时自动关闭。</user-prompt-submit-hook>" \
      "EchoCoding 平衡模式已开启：如果这次回复有重要信息、阶段性结论或需要用户注意的内容，请先执行 \`echocoding say\` 用一句话播报，再输出完整文字。⚠️ 需要反问用户时，必须用 \`echocoding ask\`。若需求未澄清，请连续多轮 ask（HUD 会话按最后互动 60 秒超时）。"
    ;;
  verbose)
    emit_reminder \
